...
The other things that were done here include the slowing down of frames using `fps` (frames per second) and adjusting the `range` in `trasition_reveal()` to be longer than the rounds it's transitioning over (ie adjusting the range to `c(0,25)`). This allows the animation to pause after it has finished its cycle.
